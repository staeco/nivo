...
import { annotationSpecPropType, useAnnotations, Annotation } from '@nivo/annotations';
import { area, curveBasis, curveLinear, line } from 'd3-shape';
...
var PartTooltip = memo(function (_ref) {
  var part = _ref.part;
...
var computeShapeGenerators = function computeShapeGenerators(interpolation, direction) {
  var areaGenerator = area().curve(interpolation === 'smooth' ? curveBasis : curveLinear);
  ...
  return [areaGenerator, line().defined(function (d) {
    return d !== null;
  }).x(function (d) {
    return d.x;
  }).y(function (d) {
...
var computeScales = function computeScales(_ref) {
  ...
  var linearScale = scaleLinear().domain([0, Math.max.apply(Math, _toConsumableArray(allValues))]).range([0, linearScaleSize]);
...
var computePartsHandlers = function computePartsHandlers(_ref3) {
  ...
  return parts.map(function (part) {
    var boundOnMouseEnter = function boundOnMouseEnter(event) {
      ...
      showTooltipFromEvent(React.createElement(PartTooltip, {
        part: part
      }), event);
...
var useFunnel = function useFunnel(_ref4) {
  ...
  var getLabelColor = useInheritedColor(labelColor, theme);
  ...
  var partsWithHandlers = useMemo(function () {
...
var PartLabels = function PartLabels(_ref) {
  var parts = _ref.parts;
  return parts.map(function (part) {
    return React.createElement(PartLabel, {
      key: part.data.id,
      part: part
    });
  });
...
var Separator = function Separator(_ref) {
  ...
  return React.createElement(animated.line, Object.assign({
    x1: animatedProps.x1,
    x2: animatedProps.x2,
    y1: animatedProps.y1,
    y2: animatedProps.y2,
...
var Separators = function Separators(_ref) {
  ...
  return React.createElement(React.Fragment, null, beforeSeparators.map(function (separator) {
...
var Funnel = function Funnel(props) {
  ...
  var layerById = {
    separators: React.createElement(Separators, {
      key: "separators",
      beforeSeparators: beforeSeparators,
      afterSeparators: afterSeparators
    }),
    parts: React.createElement(Parts, {
      key: "parts",
      parts: parts,
      areaGenerator: areaGenerator,
      borderGenerator: borderGenerator,
    ...
    annotations: React.createElement(FunnelAnnotations, {
      key: "annotations",
      parts: parts,
      annotations: annotations,
      widh: innerWidth,
  ...
  if (enableLabel === true) {
    layerById.labels = React.createElement(PartLabels, {
      key: "labels",
      parts: parts
    });
  }
...
var WrappedFunnel = withContainer(Funnel);
...
var ResponsiveFunnel = function ResponsiveFunnel(props) {
  return React.createElement(ResponsiveWrapper, null, function (_ref) {
    var width = _ref.width,
        height = _ref.height;
    return React.createElement(WrappedFunnel, Object.assign({
      width: width,
      height: height
    }, props));
  });
...
export { WrappedFunnel as Funnel, FunnelDefaultProps, FunnelPropTypes, ResponsiveFunnel, computePartsHandlers, computeScales, computeSeparators, computeShapeGenerators, useFunnel, useFunnelAnnotations };
